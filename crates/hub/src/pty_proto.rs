...
mod browser_tests {
    ...
    #[test]
    fn browser_rpc_client_to_hub_roundtrips_byte_exact() {
        let original = r#"{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{"zebra":1,"alpha":2}}"#;
        let frame = ClientToHub::BrowserRpc { payload: original.to_string() };
    ...
    #[test]
    fn browser_rpc_hub_to_client_roundtrips_byte_exact() {
        let original = r#"{"result":{"b":1,"a":2}}"#;
        let frame = HubToClient::BrowserRpc { payload: original.to_string() };
        let json = serde_json::to_string(&frame).unwrap();
