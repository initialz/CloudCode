//! Transport copy of client/wire.rs — NOT a protocol mirror (types come
//! from cloudcode-proto). Extract a shared transport crate if a 3rd
//! consumer appears.
//!
...
use bytes::Bytes;
use cloudcode_proto::{ClientToHub, HubToClient, PTY_PROTOCOL_VERSION};
...
mod tests {
    ...
    fn http_becomes_ws() {
        assert_eq!(
            build_ws_url("http://localhost:7100").unwrap(),
            "ws://localhost:7100/v1/pty/ws"
        );
    }
    ...
    fn https_becomes_wss_and_trailing_slash_trimmed() {
        assert_eq!(
            build_ws_url("https://hub.example.com/").unwrap(),
            "wss://hub.example.com/v1/pty/ws"
        );
    }

    #[test]
    fn missing_scheme_errors() {
        assert!(build_ws_url("localhost:7100").is_err());
    }

    #[test]
    fn unsupported_scheme_errors() {
        assert!(build_ws_url("ftp://host").is_err());
    }
}
