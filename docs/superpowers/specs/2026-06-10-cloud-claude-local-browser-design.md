...
│   │ stdio MCP        │         │ +路由      │         │   ├ MCP 子进程管理      │
│   ▼                  │         │  Browser  │         │   │  @playwright/mcp    │
│ cc-mcp-proxy (新)    │◄──ws───►│  Rpc 帧   │◄──ws───►│   ├ 授权门状态机        │
│   stdio⇄BrowserRpc    │         └──────────┘         │   └ handoff 控制器       │
└──────────────────────┘                              │      headless⇄headed     │
                                                       │   ▼                      │
                                                       │ 本地 Chrome              │
                                                       └────────────────────────┘
...
| 单元 | 职责 | 接口 | 依赖 |
|------|------|------|------|
| **cc-mcp-proxy**(agent,新) | claude 眼里的本地 stdio MCP server;双向搬运 JSON-RPC 帧到 hub;在 `tools/list` 层做能力过滤 + 注入 `request_handoff` | stdin/stdout(对 claude)↔ `BrowserRpc`(对 hub) | agent 现有 ws 连接 |
...
- **cc-mcp-proxy 完全无状态**,纯管道 + tools/list 改写。授权、handoff、浏览器状态全在 client 侧 —— agent 侧改动最小,敏感逻辑离用户最近。
...
```
claude ──stdio──► cc-mcp-proxy ──ClientMsg::BrowserRpc──► hub
                          ...
                          @playwright/mcp ──► 本地 Chrome
                               │ 结果
                               ▼
cc-mcp-proxy ◄──ServerMsg::BrowserRpc── hub ◄──ClientToHub::BrowserRpc── cc-browser
   │
   ▼ stdout 还给 claude
```
...
- **授权失效映射**:`BrowserClosed` 到 cc-mcp-proxy 后,被翻译成一个 MCP 错误响应丢给 claude 当前在飞的调用。
...
| client 没装 Node/Playwright | **能力协商**:client 连接上报 `browser_capable`;cc-mcp-proxy 据此决定 tools/list 是否暴露浏览器工具 | 无能力时浏览器工具不出现,claude 不会调注定失败的工具 |
...
**两个边界设计点**:
- **能力协商在 `tools/list` 层**:cc-mcp-proxy 收到 `tools/list` 时,若无 browser-capable client 绑定,从响应里**摘掉**浏览器工具(而非返回错误工具)。client 上/下线 → 主动推 `notifications/tools/list_changed`,claude 动态感知能力增减。
...
| | ③ cc-mcp-proxy 透传 | 喂 stdin 字节 → 断言帧原样;反向同理 |
...
- `crates/agent/`:新增 cc-mcp-proxy(可作 `cloudcode-agent` 子命令或独立 bin);`tunnel.rs` 加 `BrowserRpc`/`BrowserClosed` 变体;ws 路由接入。
...
- client 侧新依赖:Node + `@playwright/mcp`(随 client 安装脚本分发或运行时拉起)。
- 配置:空闲超时、接管超时、子进程重启上限等进 client 配置。

## 未决 / 留待实现细化

- cc-mcp-proxy 作为 `cloudcode-agent` 子命令 vs 独立二进制 —— 写计划时定。
- 本地确认 UI 形态(终端提示 vs 系统通知 vs cc-browser 自带小窗)—— 与 client 现有 TUI 交互方式对齐后定。
- `@playwright/mcp` 的分发方式(随安装脚本预装 vs 首次用时拉起)—— 与现有 install.sh 流程对齐。
